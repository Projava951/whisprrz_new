var CMessages = function(guid, imHistoryMessages) {
	...
	this.messageMenuShow = function(id){

		$this.messageMenuDeleteAll(id);

		var actionButton = $('#message_info_module_link_' + id);
		if(actionButton.hasClass('animating')) {
			return;
		} else {
			actionButton.addClass('animating');
		}

		var isMenuInitialized = actionButton.hasClass('initialized');

		if(!isMenuInitialized) {
			$('#message_info_module_menu').clone().prop('id', 'message_info_module_menu_' + id).appendTo('#message_info_module_' + id);
		}

		var $menu = $('#message_info_module_menu_' + id);
		$menu.click(function(){
			actionButton.click();
		});
		...
		if(!isMenuInitialized) {
			$menu.position({
				...
				using: function(coords){
					...
					actionButton.addClass('initialized message_menu_active');
					$menu.addClass('initialized');
					$this.messageMenuAnimate($menu, actionButton);
				}
				});
		} else {
			$this.messageMenuAnimate($menu, actionButton);
	...
	this.messageMenuAnimate = function($menu, actionButton){
		$menu.fadeToggle({complete: function(){
				actionButton.removeClass('animating');
			}
		});
	}

	this.messageMenuDeleteAll = function(id){

		var id = id || 0;

		$('.message_menu_link').each(function(){
			var itemId = $(this).data('id');
			if(itemId != id && ($(this).hasClass('initialized') || $(this).hasClass('animating'))) {
				$(this).removeClass('animating initialized');
				var itemMenu = $('#message_info_module_menu_' + itemId);
				if(itemMenu.length) {
					itemMenu.remove();
				}
			}
    ...
    $(function(){
        $this.init();
        if(activePage != 'messages.php')return;
        $('body').on('click', '.to_one_chat, .im_user_photo, .to_profile, .photo_grant_access, .photo_deny_access, .one_chat_to_profile_user, .im_audio_message > .im_audio_message_loader, .st-container, .message_menu_link, .message_info_module_menu', function(e){
            var el=$(this);

			if (el.is('.im_audio_message_loader')) {
console.log(this)
				playImAudioMessage(el);
			...
			if(el.is('.message_menu_link') || el.is('.message_info_module_menu')) {
				return false;
			}

			if(el.is('.st-container')) {
				$this.messageMenuDeleteAll();
			}

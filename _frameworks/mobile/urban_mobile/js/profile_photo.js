var CProfilePhoto = function() {
    ...
    this.setCachePhoto = function(id,private,main,isVideo,approve,gif) {
            ...
            $.post(activePage,{cmd:'publish_one_photo',photo_id:pid,ajax:1},
                function(res){
                    ...
                    if (data){
                        ...
                        $this.setCachePhoto(pid,private,0,0,(data === 'Y' ? 0 : 1),isGif);
...
// resize
	...
	this.isNeedLiveUpgrade = function(live_id, my_video){
		if (!userAllowedFeature['live_streaming'] && live_id && !my_video){
			showConfirm(l('watch_pasts_stream_need_upgrade'),l('upgrade'),l('cancel'),function(){redirectUrl(urlPagesSite.upgrade)},'red','#st-container',false,true);
			return true;
		}
		return false;
	}
	...
	this.checkNeedLiveCredit = function(vid, live_id, my_video, call){
		if (!live_id || my_video || !$this.live_price) {
			call();
			return false;
		}

		var $layer=$('#video_'+vid).closest('div');
		$layer[0] && $layer.addChildrenLoader();

		var liveViewAllowed = function(){
			var $btn=$('.btn_done:visible').addChildrenLoader();
			...
			if (data){
				...
				if(balance<$this.live_price){
					...
					if(typeof isInAppPurchaseEnabled != 'undefined' && !isInAppPurchaseEnabled){
						...
						showConfirm(msg,l('btn_buy_credits'),l('cancel'),function(){redirectUrl(url)},'red');
					}
				} else {
					//This service costs *** credits
					var msg=l('this_service_costs_credits').replace('{credit}',balance)
							.replace('{price}',$this.live_price);
					showConfirm(msg,l('ok'),l('cancel'),liveViewAllowed,'blue','#st-container',false,true);
				}
			}else{
				tools.showServerError();
			}
			$this.checkNeedLiveCreditAction[vid]=0;
			$layer[0] && $layer.removeChildrenLoader();
		})
	}

    this.playVideo = function(pid,live_id,my_video)
    {
        pid = pid || $this.curPid;
		live_id = live_id || 0;
		my_video = my_video || 0;

		if ($this.isNeedLiveUpgrade(live_id, my_video)) {
			return false;
		}
    ...
    this.editPhotoInfo = function(el,pid) {
        ...
        if(isVideo){
			...
			var photo = $('#photo_' + pid + ' img').attr('src').replace('_mm.jpg', '_r.jpg');//url_files + 'photo/'+$this.guid+'_'+pid+'_r.jpg',v=$this.versionPhoto;
        }
    ...
    this.prepareActionReports = function(pid, dur){
        if (!/encounters|rate_people|profile/.test($this.display)) {
            return;
        }
    ...
    this.setPhotoInfo = function(pid){
		...
		if ($this.uid == $this.guid && 'profile' == $this.display) {
			removeChildrenLoader($this.$btnImageEdit);
			removeChildrenLoader($this.$btnAdditional.find('.pp_gallery_restore_image'));

			$this.controlRestoreImage($this.photosInfo[pid]['restore']);
			$this.$btnAdditional[$this.photosInfo[pid]['gif']?'removeClass':'addClass']('to_show');
		} else {
			$this.$btnAdditional&&$this.$btnAdditional[0]&&$this.$btnAdditional.removeClass('to_show');
		}
    ...
    this.replacePhotoAfterEditor = function(pid, sel) {
        sel=sel||'#photo_'+pid;
        var i=0;
        $(sel).each(function(){
            var $img=$(this), v=+new Date, image=new Image();
            var size='s',//$img.data('photo-size'),
				src;
            src=size==undefined?$img.attr('src'):urlFiles+$this.photosInfo[pid]['src_'+size];
	...
	this.refreshImage = function(pid, edit, restore){
		if($this.photosInfo[pid] != undefined){
            var photo=$this.photosInfo[pid]['user_id']+'_'+pid,
                sizes=['b', 's', 'r', 'm', 'bm'],
				preloadArr=[],i=0,url,v=+new Date; v='?v='+v;
            sizes.forEach(function(size,i,arr) {
                //url='photo/'+photo+'_'+size+'.jpg'+v;
                $this.photosInfo[pid]['src_'+size] = urlAddUniqueVersionParam($this.photosInfo[pid]['src_'+size]);
                preloadArr[i++] = urlFiles + $this.photosInfo[pid]['src_'+size];
            })
            ...
            $this.replacePhotoAfterEditor(pid, '#menu_user_photo_profile');
		}
	}

	this.updateRestoreImage = function(pid, restore){
		if (typeof $this.photosInfo[pid] != 'undefined') {
			$this.photosInfo[pid]['restore']=restore;
		}
		if (pid == $this.curPid) {
	...
	this.restoreImage = function($btn){
		...
		showConfirm(l('all_changes_will_be_lost'), l('ok'), l('cancel'), fn, 'blue');
	}
    ...
    $(function(){
        ...
        if (activePage == 'profile_view.php'|| activePage == 'search_results.php') {
            ...
            if (/profile|encounters/.test($this.display)) window.$main=$main.css({overflow: 'hidden', position: 'absolute'}).swipe({
                ...
                excludedElements: 'a, :input, .noSwipe, #tip_report, .pp_gallery_restore_image'
            });

var CLiveStreaming = function(guid) {
    ...
    this.initStyle = function(){
        var fn=function(){
            ...
            $jq('#ls_custom_style')[0].innerHTML=[
                ".wall_video_post .one_media_vimeo{height:", Math.round(w/d), "px; width: 100%;}",
                ".wall_video_post .one_media_youtube{height:", Math.round(w/d), "px; width: 100%;}",
                ".wall_video_post .one_media_metacafe{height:", Math.round(w/d), "px; width: 100%;}",
                ".wall_video_one_post .one_media_vimeo{height:", Math.round((w-80)/d), "px; width: 100%;}",
                ".wall_video_one_post .one_media_youtube, .wall_video_one_post .one_media_metacafe{height:", Math.round((w-80)/d), "px; width: 100%;}"
                ].join("");
    ...
    this.likeAdd = function($link, like) {
		...
		$.post(url_ajax+'?cmd=live_stream_like',{id:$this.lsVideoId, like:like}, function(data){
            ...
            if(data !== false){
                var $likes=$(trim(data)).filter('.who_liked');

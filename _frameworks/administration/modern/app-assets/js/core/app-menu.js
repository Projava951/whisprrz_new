  ...
  $.app.nav = {
    ...
    bind_events: function() {
      ...
      $('.navigation-main').on('mouseenter.app.menu', 'li', function() {
        ...
        var $listItem = $(this);
		/* Mod main menu */
		//if ($listItem.closest('#main-menu-navigation')[0]) {
			//return true;
		//}
        ...
        if($listItem.is('.disabled')){
          ...
          if( ($body.hasClass('menu-collapsed') && $body.data('menu') != 'vertical-menu-modern') ){
            ...
            if ($listItem.has('ul')) {
				...
				var isMainMenu=$listItem.closest('#main-menu-navigation')[0],
					aLink='';
				if (isMainMenu) {
					aLink=$listItem.children('a').attr('href');
					if (aLink == undefined) {
						aLink='';
					}
				}
				...
				if ($listItem.is('.open')) {
					...
					if (aLink) {
						$listItem.oneTransEnd(function(){
							setTimeout(function(){redirectUrl(aLink)},100);
						})
					}
				}
				/* Mod main menu */
      ...
      $('.navigation-main li.has-sub > a').on('click',function(e){
			/* Mod main menu */
			/*var $a=$(this);
			if ($a.closest('.nav-item')[0] && $a.closest('#main-menu-navigation')[0]) {
				return true;
			}
			/* Mod main menu */
			e.preventDefault();
          ...
          if ($listItem.has('ul')) {
            ...
            if ($listItem.is('.open')) {
              ...
              // If menu accordion then close all except clicked once
                ...
                $listItem.siblings('.open').trigger('close.app.menu');
				/* Mod main menu */
				var aLink = '', $aLink =$(this).find('a.menu-item');
				if ($aLink[0]) {
					aLink = $aLink[0].href;
				}
				if (aLink) {

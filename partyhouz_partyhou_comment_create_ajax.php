...
function do_action()
{
	...
	$event_id = intval(get_param('partyhou_id'));
	$comment_text = get_param('comment_text');

	if($partyhou_id && $comment_text)
	{
        $partyhou = DB::row("SELECT * FROM partyhouz_date WHERE partyhou_id=" . to_sql($partyhou_id, 'Number') . " LIMIT 1");
        if($partyhou)
        {
            DB::execute("INSERT INTO partyhouz_date_comment SET partyhou_id=".$partyhou['partyhou_id'].

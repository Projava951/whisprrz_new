...
class CVidsTools
{
    ...
    /* Comments to video */
	static public function getWhereFilterComments($where = '')
	{
	...
	{
		$where = self::getWhereFilterComments($where);
	...
	{
		$where = '`video_id` = ' . to_sql($video_id);
		$where = self::getWhereFilterComments($where);
	...
	static public function getCommentsByVideoId($video_id, $limit = '')
	{
        ...
        $rows_r = array();
        foreach ($rows as $k => $v) {
			$v['text'] = ImAudioMessage::getHtmlPlayer($v, $v['id'],  'video_comment_audio_', true) . $v['text'];
	...
	static public function insertCommentByVideoId($video_id, $text = null, $date = null, $isAddWall = true, $videoUserId = null, $getInfo = false, $groupId = 0, $groupUserId = 0)
	{
        ...
        if (is_array($video)) {
			...
			$text = Wall::addCommentPrepare($text, false);
			$imageUpload = get_param_int('image_upload');
			if (!$imageUpload && !Common::isOptionActiveTemplate('gallery_comment_parse_media')) {
			...
			ImAudioMessage::updateImMsgId($audioMessageId, $r, 'video_comment_id');
	...
	static public function delCommentById($id)
	{
        ...
        if (is_array($comment)) {
            ...
            if (is_array($video)) {
				...
				ImAudioMessage::delete($comment['id'], $comment['user_id'], 'video_comment_id');
                ...
                if ($comment['parent_id']) {
                    ...
                    foreach ($parentComments as $key => $item) {
						ImAudioMessage::delete($item['id'], $item['user_id'], 'video_comment_id');

                        Wall::remove('vids_comment', $item['id'], $item['user_id']);
						Wall::removeImgComment($item['text']);

...
*/
    ...
    static function partyhouz_by_user_sql_base($user_id)
    {
        ...
        $sql = "partyhouz_partyhou as e LEFT JOIN user u ON u.user_id = e.user_id WHERE e.user_id=" . to_sql($user_id, 'Number') .
    ...
    static function partyhouz_by_user_as_guest_sql_base($user_id)
    {
        ...
        $sql = "partyhouz_partyhou as e LEFT JOIN user u ON u.user_id = e.user_id, partyhouz_partyhou_guest as g WHERE e.partyhou_id = g.partyhou_id AND g.user_id=" . to_sql($user_id, 'Number') .
            " ORDER BY e.partyhou_datetime DESC";
    ...
    static function partyhouz_most_discussed_sql_base($where="")
    {
        ...
        $sql = "partyhouz_partyhou as e LEFT JOIN user u ON u.user_id = e.user_id WHERE $where DATE_ADD(e.partyhou_datetime, INTERVAL 3 HOUR) > NOW() " .
    ...
    static function partyhouz_most_anticipated_sql_base()
    {
        ...
        $sql = "partyhouz_partyhou as e LEFT JOIN user u ON u.user_id = e.user_id WHERE DATE_ADD(e.partyhou_datetime, INTERVAL 3 HOUR) > NOW() " .
	        " ORDER BY " . ($order_by_from_settings ? ($order_by_from_settings . ", ") : '') .
    ...
    static function partyhouz_by_calendar_day($day_time)
    {
        global $g_user;
        ...
        $sql = "partyhouz_partyhou as e
                ...
                " AND e.partyhou_datetime >= DATE_SUB('" . date("Y-m-d", $day_time) . "', INTERVAL 1 DAY) " .
                " ORDER BY e.partyhou_datetime ASC " . ($order_by_from_settings ? ("," . $order_by_from_settings . "") : '');
    ...
    static function partyhouz_by_place_sql_base($place, $upcoming)
    {
        ...
        $sql = "partyhouz_partyhou as e LEFT JOIN user u ON u.user_id = e.user_id WHERE DATE_ADD(e.partyhou_datetime, INTERVAL 3 HOUR) " . ($upcoming ? '>' : '<=') . " NOW() AND " .
            "e.partyhou_place LIKE " . to_sql($place) .
            " ORDER BY " .
            ($order_by_from_settings ? ($order_by_from_settings . ", ") : '') .
            " e.partyhou_datetime " . ($upcoming ? 'ASC' : 'DESC') . ", e.created_at DESC";
    ...
    static function partyhouz_by_partyhou_datetime_sql_base($partyhou_datetime, $upcoming)
    {
		...
		global $g_user;
        $sql = "partyhouz_partyhou as e LEFT JOIN user u ON u.user_id = e.user_id WHERE DATE_ADD(e.partyhou_datetime, INTERVAL 3 HOUR) " . ($upcoming ? '>' : '<=') . " NOW() AND " .
            "e.partyhou_datetime >= DATE(" . to_sql($partyhou_datetime) . ") AND e.partyhou_datetime < DATE_ADD(DATE(" . to_sql($partyhou_datetime) . "), INTERVAL 1 DAY) " .
            " ORDER BY " .
            ($order_by_from_settings ? ($order_by_from_settings . ", ") : '') .
            " e.partyhou_datetime " . ($upcoming ? 'ASC' : 'DESC') . ", e.created_at DESC";
    ...
    {
        $order_by_from_settings = self::order_by_from_settings();
        $sql = "partyhouz_partyhou as e LEFT JOIN user u ON u.user_id = e.user_id WHERE DATE_ADD(e.partyhou_datetime, INTERVAL 3 HOUR) " . ($upcoming ? '>' : '<=') . " NOW() ORDER BY RAND()";
        return array('query' => $sql, 'columns' => 'e.*, u.name');
    }

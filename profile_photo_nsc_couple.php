...
class CPhoto extends CHtmlBlock
{
	...
	function action()
	{
		...
		if ($cmd == "insert")
		{
			...
			if ($this->message == "")
			{
				...
				uploadphoto($nsc_couple_id, $photo_name, $description, $g['options']['photo_approval']=='Y'?0:1);
                redirect();
			}
		}
		...
		if ($cmd == "update")
		{
			...
			else
			{
				...
				DB::execute("UPDATE photo SET
					...
					WHERE photo_id=" . to_sql($photo_id, "Number") . " AND user_id=" . $nsc_couple_id . ";"
				);
			}
		}
	...
	function parseBlock(&$html)
	{
		...
		$num_photos = DB::result("SELECT COUNT(photo_id) FROM photo WHERE user_id=" . $nsc_couple_id . " AND visible != 'P'  AND group_id = 0");
		$html->setvar("num_photos", $num_photos);
        $html->setvar('name', $g_user['name']);

		DB::query("SELECT * FROM photo WHERE user_id=" . $nsc_couple_id . "  AND visible != 'P' AND group_id = 0 ORDER BY photo_id ASC");
		...
		for ($i = 1; $i <= $num_photos; $i++)
		{
			...
			if ($row = DB::fetch_row())
			{
                ...
                $html->setvar("photo_offset", User::photoOffset($nsc_couple_id, $row['photo_id']));
...
g_user_full();
$page = new CPhoto("", $g['tmpl']['dir_tmpl_main'] . "profile_photo_nsc_couple.html");

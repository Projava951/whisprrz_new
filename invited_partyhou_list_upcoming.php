...
try
{
	$sql = "SELECT pc.category_id, 
			...
			CONCAT(
				LPAD(FLOOR(TIME_TO_SEC(TIMEDIFF(pp.partyhou_datetime, NOW())) / 60) - 5, 2, '0'),
				':',
			...
			WHERE ppg.user_id='".$user_id."' AND ppg.declined = '0' AND NOW() >= pp.partyhou_datetime - INTERVAL 15 MINUTE AND NOW() <= pp.partyhou_datetime - INTERVAL 5 MINUTE";

		...
		if ($cmd == "update")
		{
	        ...
	        if($partyhou = DB::fetch_row())
	        {
                      DB::query("SELECT m.*, cn.*, st.*, ct.* " .
                        "FROM partyhouz_partyhou as m, geo_country as cn, geo_state as st, geo_city as ct " .
                        "WHERE m.partyhou_id=" . to_sql($partyhou_id, 'Number') . " AND m.city_id = ct.city_id AND ct.state_id = st.state_id AND st.country_id = cn.country_id LIMIT 1");
                      if ($partyhou = DB::fetch_row()) {

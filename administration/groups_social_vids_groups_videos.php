...
class CPage extends CHtmlBlock
{
	function parseBlock(&$html)
	{
        ...
        $itemsTotal = CVidsTools::countVideosByUser($uid, ' AND `group_id` = ' . to_sql($groupId));
        $items = CVidsTools::getVideosByUser($uid, (($page - 1) * $pagerOnPage) . ',' . $pagerOnPage, ' AND `group_id` = ' . to_sql($groupId));

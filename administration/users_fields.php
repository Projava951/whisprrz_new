...
class CAdminFields extends CHtmlBlock
{
                ...
                if ($this->table != 'const_orientation') {
                    ...
                    while ($row = DB::fetch_row()) {
                        ...
                        if($this->table == 'const_lms_user_type') {
                            $this->parseLmsUserType($html, $row);
                        }
    ...
    public function parseLmsUserType($html, $row)
    {
        $lmsUserTypes = array(
            'teacher',
            'student',
        ...
        $this->parseFreeAccessField($html, $row);

        $html->parse('field_lms_user_type', false);
    }

    public function parseFreeAccessField($html, $row)
    {
        $templateSet = Common::getTmplSet();
        if ($templateSet != 'urban'){
            $types = array(
                'none' => l('None'),
                'silver' => l('Silver'),
                'gold' => l('Gold'),
        ...
        if(($templateSet == 'urban') && ($row['free'] != 'none')) {
            $row['free'] = 'platinum';
        }
        $html->setvar('field_free_access_select', h_options($types, $row['free']));

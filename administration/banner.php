...
class CBanners extends CHtmlBlock
{
	function action()
	{
        ...
        if($cmd == 'effect_save'){
			...
			$banner_effect = DB::row("SELECT * ".
            "FROM config ".   				
            "WHERE config.option='effects' AND module = 'wowslider_main'" .
			" LIMIT 1");
			if($banner_effect){
				DB::execute("UPDATE config SET " .
                    " value=".to_sql(get_param('slider_type'))." WHERE config.option='effects' AND module = 'wowslider_main'");
			}else{
				DB::execute("INSERT INTO config SET ".
                    " config.option='effects'".
                    ", module='wowslider_main'".
					", value=".to_sql(get_param('slider_type')).
                ""
                );
			}
            redirect('banner.php');
			...
			$banner_effect = DB::row("SELECT * ".
            "FROM config ".   				
            "WHERE config.option='distance' AND module = 'wowslider_main'" .
			" LIMIT 1");
			if($banner_effect){
				DB::execute("UPDATE config SET " .
						" value=".to_sql(get_param('distance'))." WHERE config.option='distance' AND module = 'wowslider_main'");
			}else{
				DB::execute("INSERT INTO config SET ".
                    " config.option='distance'".
                    ", module='wowslider_main'".
					", value=".to_sql(get_param('distance')).
                ""
                );
			}
			$banner_effect = DB::row("SELECT * ".
			...
			if($banner_effect){
				 DB::execute("UPDATE config SET " .
                    " value=".to_sql(get_param('delay_time'))." WHERE config.option='delay_time' AND module = 'wowslider_main'");
			}else{
				DB::execute("INSERT INTO config SET ".
                    " config.option='delay_time'".
                    ", module='wowslider_main'".
					", value=".to_sql(get_param('delay_time')).
                ""
                );
			}
			$banner_effect = DB::row("SELECT * ".
			...
			if($banner_effect){
				DB::execute("UPDATE config SET " .
                    " value=".to_sql(get_param('duration_time'))." WHERE config.option='duration_time' AND module = 'wowslider_main'");
			}else{
				DB::execute("INSERT INTO config SET ".
                    " config.option='duration_time'".
                    ", module='wowslider_main'".
					", value=".to_sql(get_param('duration_time')).
                ""
                );
			}
            $banner_effect = DB::row("SELECT * ".
			...
			if($banner_effect){
				DB::execute("UPDATE config SET " .
                    " value=".to_sql(get_param('title_size'))." WHERE config.option='title_size' AND module = 'wowslider_main'");
			}else{
				DB::execute("INSERT INTO config SET ".
                    " config.option='title_size'".
                    ", module='wowslider_main'".
					", value=".to_sql(get_param('title_size')).
                ""
                );
			}
			$banner_effect = DB::row("SELECT * ".
			...
			if($banner_effect){
				DB::execute("UPDATE config SET " .
                    " value=".to_sql(get_param('title_color'))." WHERE config.option='title_color' AND module = 'wowslider_main'");
			}else{
				DB::execute("INSERT INTO config SET ".
                    " config.option='title_color'".
                    ", module='wowslider_main'".
					", value=".to_sql(get_param('title_color')).
                ""
                );
			}
            $banner_effect = DB::row("SELECT * ".
			...
			if($banner_effect){
				DB::execute("UPDATE config SET " .
                    " value=".to_sql(get_param('title_back_color'))." WHERE config.option='title_back_color' AND module = 'wowslider_main'");
			}else{
				DB::execute("INSERT INTO config SET ".
                    " config.option='title_back_color'".
                    ", module='wowslider_main'".
					", value=".to_sql(get_param('title_back_color')).
                ""
                );
			}
            $banner_effect = DB::row("SELECT * ".
			...
			if($banner_effect){
				DB::execute("UPDATE config SET " .
                    " value=".to_sql(get_param('title_bottom'))." WHERE config.option='title_bottom' AND module = 'wowslider_main'");
			}else{
				DB::execute("INSERT INTO config SET ".
                    " config.option='title_bottom'".
                    ", module='wowslider_main'".
					", value=".to_sql(get_param('title_bottom')).
                ""
                );
			}
			$banner_effect = DB::row("SELECT * ".
			...
			if($banner_effect){
				DB::execute("UPDATE config SET " .
                    " value=".to_sql(get_param('econtrol_size'))." WHERE config.option='control_size' AND module = 'wowslider_main'");
			}else{
				DB::execute("INSERT INTO config SET ".
                    " config.option='control_size'".
                    ", module='wowslider_main'".
					", value=".to_sql(get_param('econtrol_size')).
                ""
                );
			}
			$banner_effect = DB::row("SELECT * ".
			...
			if($banner_effect){
				if(!is_null(get_param('econtrol_state')) && get_param('econtrol_state')==1){
					DB::execute("UPDATE config SET " .
						" value=".to_sql(get_param('econtrol_state'))." WHERE config.option='control_visible' AND module = 'wowslider_main'");
				}else{
					DB::execute("UPDATE config SET " .
							" value=0 WHERE config.option='control_visible' AND module = 'wowslider_main'");
				}
			}else{				
				if(!is_null(get_param('econtrol_state')) && get_param('econtrol_state')==1){
					DB::execute("INSERT INTO config SET ".
						" config.option='control_visible'".
						", module='wowslider_main'".
						", value=".to_sql(get_param('econtrol_state')).
					""
					);
				}else{
					DB::execute("INSERT INTO config SET ".
			...
			if($banner_effect){
				if(!is_null(get_param('title_sliding_state')) && get_param('title_sliding_state')==1){
					DB::execute("UPDATE config SET " .
							" value=".to_sql(get_param('title_sliding_state'))." WHERE config.option='title_sliding_state' AND module = 'wowslider_main'");
				}else{
					DB::execute("UPDATE config SET " .
							" value=0 WHERE config.option='title_sliding_state' AND module = 'wowslider_main'");
				}
			}else{				
				if(!is_null(get_param('title_sliding_state')) && get_param('title_sliding_state')==1){
					DB::execute("INSERT INTO config SET ".
						" config.option='title_sliding_state'".
						", module='wowslider_main'".
						", value=".to_sql(get_param('title_sliding_state')).
					""
					);
				}else{
					DB::execute("INSERT INTO config SET ".
	...
	function parseBlock(&$html)
	{
		...
		$event = DB::row("SELECT e.*, cn.*, ct.*, st.* ".
            "FROM wowslider_main as e,user u, geo_country as cn, geo_state as st, geo_city as ct ".   				
		...
		*/
        ...
        DB::query("SELECT e.* FROM wowslider_main as e WHERE 1 = 1 ".(($event_id)?("AND e. event_id=".$event_id):"")." ORDER BY e.event_id DESC");
...
$page = new CBanners('', $g['tmpl']['dir_tmpl_administration'] . 'banner.html');

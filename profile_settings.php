...
UserFields::$parseTextDescrption = true;
...
class CProfileSettings extends SettingsField
{
	...
	function action()
	{
        ...
        if ($cmd == 'update_email')
		{
            ...
            if (!User::passwordVerify($password, $g_user['password'])){
	...
	function parseBlock(&$html)
	{
        ...
        if ($html->blockexists($block)
            ...
            Pay::parseGooglePlaySubscrionManageButton($html);

            $html->parse($block);
        }

		$block = 'st_credits_balans';
		if ($html->blockexists($block) && Common::isCreditsEnabled()) {
			$html->setvar('st_credits_balans', lSetVars('credit_balance', array('credit' => $g_user['credits'])));
			$html->parse($block);
		}
...
//    $profileEditor = new CProfileEditMain('profile_edit_main',  $g['tmpl']['dir_tmpl_main'] . "_profile_edit_main.html", false, false, false, 'birthday', guid());
//
//    $profileEditorPersonal = new CProfileEditMain('profile_edit_personal',  $g['tmpl']['dir_tmpl_main'] . "_pp_profile_edit_personal.html", false, false, false, 'personal_edit_urban', guid());
//    $profileEditor->add($profileEditorPersonal);
//
//    $profileEditorAbout = new CProfileEditMain('profile_edit_about', $g['tmpl']['dir_tmpl_main'] . "_profile_edit_about.html", false, false, false, 'personal_edit_urban_mobile', guid());
//    $profileEditorAbout::$parseTextDescrption = true;
//    $profileEditor->add($profileEditorAbout);
//
//    $profileEditorLookingFor = new CProfileEditMain('profile_edit_looking_for', $g['tmpl']['dir_tmpl_main'] . "_pp_edit_looking_for.html", false, false, false, 'edit_looking_for_urban', guid());
//    $profileEditorLookingFor->customDebugField = true;
//    $profileEditor->add($profileEditorLookingFor);
//
//    $profileEditorLookingFor->init();

...
function do_action()
{
	...
	$event_id = intval(get_param('hotdate_id'));
	$comment_text = get_param('comment_text');

	if($hotdate_id && $comment_text)
	{
        $hotdate = DB::row("SELECT * FROM hotdates_date WHERE hotdate_id=" . to_sql($hotdate_id, 'Number') . " LIMIT 1");
        if($hotdate)
        {
            DB::execute("INSERT INTO hotdates_date_comment SET hotdate_id=".$hotdate['hotdate_id'].
